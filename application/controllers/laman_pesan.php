...
class laman_pesan extends CI_Controller {
    ...
    //         // Debugging: Check if id_kamar exists in the kamar table
    //         $kamar_exists = $this->db->get_where('kamar', array('id' => $id_kamar))->num_rows();
    //         if ($kamar_exists == 0) {
    //             log_message('error', 'No kamar with id ' . $id_kamar . ' exists in the kamar table.');
    //             show_error('Selected room does not exist.');
    //             return;
    //         }
    ...
    public function TambahPemesanan($id = null) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Ensure at least one day is counted if the check-in and check-out dates are the same
            if ($days == 0) {
                $days = 1;
            }
